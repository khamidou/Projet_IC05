...
#         gexfXML.set("xmlnsxsi",)
    ...
    def write(self,file):
        fileStr = str(etree.tostring(self.getXML(),pretty_print=True,encoding="utf-8",xml_declaration=True))
        fileStr = fileStr.replace("b\'", '')
...
class Graph :
    ...
    def nodeExists(self,id) :
        if id in self._nodes.keys():
            ...
            return 0
    def nodeValueExists(self,id) :
        if id in self._nodes.values():
            return 1
        else :
            return 0
...
class Edge :
    ...
    def __init__(self,graph,id,source,target,weight="",start="",end="",label="",r="",g="",b="",spells=[],startopen=False,endopen=False) : 
        ...
        if self._graph.nodeValueExists(source) :
            self._source=source
            self.source=self._source
        else :
            raise Exception("source "+source+" node unknown, add nodes to graph first")
            
        if self._graph.nodeValueExists(target) :
            self._target=target
